...
class PropertyController extends AdminController
{
    ...
    protected function grid()
    {
        ...
        $grid->column('customer_name', __('Tên khách/doanh nghiệp'))->width(200);
        $grid->column('property_type', __('Loại tài sản'))->using(Constant::PROPRERTY_TYPE)->width(200);
        $grid->column('address', __('Địa điểm tài sản'))->using(Constant::PROPRERTY_ADDRESS)->width(200);
        $grid->column('purpose', __('Mục đích sử dụng đất'))->using(Constant::PROPRERTY_PURPOSE)->width(200);
        $grid->column('ptvt_type', __('Loại hình PTVT'))->using(Constant::VEHICLE_TYPE)->width(200);
        ...
        $grid->column('business', __('Ngành nghề'));
        $grid->column('branch.branch_name', __('Chi nhánh'));
        $grid->column('created_at', __('Ngày tạo'))->width(100);
        $grid->column('updated_at', __('Ngày cập nhật'))->width(100);
